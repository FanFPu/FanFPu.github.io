...
od <- '/jdfsbjcas1/ST_BJ/P21H28400N0232/xieguixiang/BLCA/TSanalysis/CellChat/TS1vs4/'
setwd(od)

x <- load('/jdfsbjcas1/ST_BJ/P21H28400N0232/xieguixiang/BLCA/TSanalysis/CellChat/TS1/cellchat_interaction.RData')
cellchat_high_TIL <- get(x)
y <- load('/jdfsbjcas1/ST_BJ/P21H28400N0232/xieguixiang/BLCA/TSanalysis/CellChat/TS4/cellchat_interaction.RData')
cellchat_low_TIL <- get(y)

cellchat <- mergeCellChat(list(cellchat_high_TIL, cellchat_low_TIL), add.names = c("TS1", "TS4"), cell.prefix = TRUE)
...
object.list <- list(TS1 = cellchat_high_TIL, TS4 = cellchat_low_TIL)
i = 1
# combining all the identified signaling pathways from different datasets 
pathway.union <- union(object.list[[i]]@netP$pathways, object.list[[i+1]]@netP$pathways)
ht1 = netAnalysis_signalingRole_heatmap(object.list[[i]], pattern = "outgoing", signaling = pathway.union, title = names(object.list)[i], width = 5, height = 16, color.heatmap = "GnBu")
ht2 = netAnalysis_signalingRole_heatmap(object.list[[i+1]], pattern = "outgoing", signaling = pathway.union, title = names(object.list)[i+1], width = 5, height = 16, color.heatmap = "GnBu")
...
net.up <- subsetCommunication(cellchat, net = net, datasets = "TS4",ligand.logFC = 0.2, receptor.logFC = NULL)
# extract the ligand-receptor pairs with upregulated ligands and upregulated recetptors in NL, i.e.,downregulated in LS
net.down <- subsetCommunication(cellchat, net = net, datasets = "TS1",ligand.logFC = -0.1, receptor.logFC = -0.1)
gene.up <- extractGeneSubsetFromPair(net.up, cellchat)
gene.down <- extractGeneSubsetFromPair(net.down, cellchat)
pairLR.use.up = net.up[, "interaction_name", drop = F]
print(pairLR.use.up)
pairLR.use.down = net.down[, "interaction_name", drop = F]
gg1 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.up, sources.use = c('Macrophage','Fibroblast','Tcell'), targets.use = c('Tumor_Epithelial'),
...
pdf(paste0(od, "up-regulated_fib&macro&T_sender_signal.pdf"),width=4,height=5)
gg1
dev.off()


gg1 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.up, sources.use = c('Tumor_Epithelial'), targets.use = c('Macrophage','Fibroblast','Tcell'),
                        comparison = c(1, 2),  angle.x = 45, remove.isolate = T,title.name = paste0("Up-regulated signaling in ", names(object.list)[2]))
...
pdf(paste0(od, "up-regulated_tumor_sender_signal.pdf"),width=5,height=7)
gg1
dev.off()

gg2 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.down, sources.use = c('Myocyte'), targets.use = c('Tumor_Epithelial'), 
                        comparison = c(1, 2),  angle.x = 45, remove.isolate = T,title.name = paste0("Down-regulated signaling in ", names(object.list)[2]))
pdf(paste0(od, "down-regulated_myocyte_sender_signal.pdf"),width=3.5,height=4)
gg2
dev.off()
gg2 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.up, sources.use = c('Myocyte','Tumor_Epithelial'), targets.use = c('Tumor_Epithelial','Myocyte'), 
                        comparison = c(1, 2),  angle.x = 45, remove.isolate = T,title.name = paste0("Up-regulated signaling in ", names(object.list)[2]))
pdf(paste0(od, "up-regulated_myocyte_epi_sender_signal.pdf"),width=5,height=5)
gg2
dev.off()
